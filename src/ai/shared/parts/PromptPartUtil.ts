...
export abstract class PromptPartUtil<T extends BasePromptPart = BasePromptPart> {
	 ...
	 * @param _part - The prompt part.
	 * @param _settings - Optional settings for custom prompt/schema.
	 * @returns The system message, or null to not add anything to the system message.
	 */
	buildSystemPrompt(_part: T, _settings?: PromptBuildSettings): string | null {
		return null
	}
}
